...
| `manifest` | `manifest_test.go` | 6 | Add/find, replace existing, save/load cycle, remove, removeAll, allSkillNames, load non-existent |
| `client` | `detect_test.go` | 3 | Detect with config dirs, detect without, ParseClientID, registry.Detected |
| **Total** | **8 files** | **31 tests** | All pass with `-race` |
