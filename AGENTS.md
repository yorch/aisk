...
- **`internal/cli`** — Cobra commands: `install`, `uninstall`, `list`, `update`, `status`, `clients`, `create`, `lint`, `audit`, `completion`. Orchestrates the full install pipeline (discover skills → detect clients → pick adapter → install → update manifest).
